...
SimpleCodeTemplate = [[
{ 
  Tiến trình   : {$ProcessName}
  Phiên bản    : 1.0
  Ngày         : {$Date}
  Tác giả      : [aamaker.lua]

  Kịch bản này làm blah blah blah
}
...
local labelCheatName = createLabel(form) labelCheatName.setPosition(16,2) labelCheatName.Caption = 'Tên cheat [tùy chọn]:'
local groupBox = createGroupBox(form) groupBox.setSize(172,96) groupBox.setPosition(4,45) groupBox.Caption = 'Loại cheat'
...
local items = cmbCheatType.getItems() items.add('Đơn giản') items.add('Tiêm') items.add('Nop') items.add('Quét mảng mô-đun') items.add('Quét mảng mô-đun lén') items.add('Vô hiệu gọi') setProperty(cmbCheatType, 'ReadOnly', true) cmbCheatType.setItemIndex(0)
local cbTestingBytes = createCheckBox(groupBox) cbTestingBytes.Name = 'cbTestingBytes' cbTestingBytes.setSize(90,19) cbTestingBytes.setPosition(3,30) cbTestingBytes.Caption = 'Kiểm tra byte' cbTestingBytes.Enabled = false
local cbOrigCodeType = createCheckBox(groupBox) cbOrigCodeType.Name = 'cbOrigCodeType' cbOrigCodeType.setSize(135,19) cbOrigCodeType.setPosition(3,54) cbOrigCodeType.Caption = 'Mã byte gốc'
local edValue = createEdit(form) edValue.setSize(68,23) edValue.setPosition(7,166) edValue.Name = 'edValue' edValue.Text = '100'
local labelValue = createLabel(form) labelValue.setPosition(7,146) labelValue.Caption = 'Giá trị:'
...
local typevalitems = cmbType.Items typevalitems.add('hex') typevalitems.add('thập phân') typevalitems.add('float') cmbType.setItemIndex(1)
local labelTypeValue = createLabel(form) labelTypeValue.setPosition(77,146) labelTypeValue.Caption = 'Loại giá trị:'
local btnOk = createButton(form) btnOk.Name = 'btnOk' btnOk.Caption = 'Tạo kịch bản' btnOk.setSize(176,24) btnOk.setPosition ( 3,195) setProperty(btnOk, 'Mặc định', true)
------------------------------------------------------
...
function getReturnFromCall(strAddress)
	...
	if string.match(opcode, "%a+") ~= 'call' then
		messageDialog("Đây không phải chức năng gọi!\r\nXin chọn cấu trúc khác!",mtWarning,mbOK)
		return false
	end
	...
	if address ~= nil then
		...
		messageDialog("Không tìm thấy địa chỉ!\r\nXin chọn cấu trúc khác!",mtWarning,mbOK)
		return false
	end
end
function getFullSignature(startAddress, stopAddress)
	...
	if #str > minNum then
		...
		if result == nil then
			print("Lỗi! Chữ ký của bạn không nằm trong một phần của mã. Không kết quả! Bạn cần một đặc quyền '+ X-C-W'")
			return ''
		end
...
function AddItemWithSubMenusInMemoryViewForm(nameItemMenu, functionSelectiontracker)
	...
	AddItemMenu(mi, 'Tạo tập hợp đơn giản', 'Ctrl+1', OnGenerateAASimpleClick)
	AddItemMenu(mi, 'Tạo tập hợp tiêm', 'Ctrl+2', OnGenerateAAInjectionClick)
	AddItemMenu(mi, 'Tạo tập hợp nop', 'Ctrl+3', OnGenerateAANopClick)
	AddItemMenu(mi, 'Vô hiệu cuộc gọi', 'Ctrl+5', OnGenerateAADisableCallClick)
	AddItemMenu(mi, 'Tạo tập hợp quét mảng mô-đun', 'Ctrl+6', OnGenerateAAAobScanModuleClick)
	AddItemMenu(mi, 'Tạo tập hợp quét mảng mô-đun lén', 'Ctrl+7', OnGenerateAAAobScanModuleStealthClick)
...
function OnItemMenuGetSignatureInfoClick(sender)
	...
	print('Địa chỉ: '..getNameFromAddress(address)..' hoặc '.. string.format('%08x', address))
	...
	if length <= 5  then
		print('Xin lỗi. Bạn phải chọn thêm 5 byte')
		return
	end
	
	for i=1, length do strSignature = strSignature .. string.format('%02X ', bytestring[i]) end
	
	print('Bắt đầu quét mảng với : '.. strSignature)
	...
	if result == nil then
		...
		result.destroy()
		print('')
		if (count == 1) then print('   ' ..'Chữ ký là duy nhất. Tốt! :) ')
		else print('   ' ..string.format('Lấy làm tiếc. Chữ ký không phải là duy nhất. :( Đã tìm thấy địa chỉ = %s', count))	end
		print('')
		strSignature = getBytesForAOB(address,length,stop)
		print('Kiểm tra chuỗi byte :  ' .. strSignature)
		print('')
	end
				
	print('Cảm ơn bạn đã sử dụng lua-plugin này, GameHackLab[RU], 2009-2013(C)')		
	print('--KẾT THÚC--')
end
------------------------------------------------------
function btnOkClick()
...
AddItemWithSubMenusInMemoryViewForm('* Tạo tập hợp nhanh', OnSelectionTracker)
AddItemMenuInMemoryViewForm('* Cửa sổ tạo tập hợp', 'Ctrl+Z', OnItemMenuCreateCheatClick, OnSelectionTracker)
AddItemMenuInMemoryViewForm('* Nhận thông tin chữ ký (trình tập hợp)', 'Ctrl+Shift+I', OnItemMenuGetSignatureInfoClick, OnSelectionTracker)