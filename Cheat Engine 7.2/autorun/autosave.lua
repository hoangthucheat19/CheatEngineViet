...
local AutoSaveSettings=getSettings('Lưu tự động')
local AutoSaveVersion=1

autosave.getPath=function()
  local path=AutoSaveSettings['Đường dẫn lưu']
...
function autosave.saveState()
  ...
  if pid and pid~='' then
    ...
    if pid~=getCheatEngineProcessID() then
      ...
      if getProcessList()[pid]==nil then
        ...
        messageDialog(translate('Một phiên bản khác của cheat engine đã bị lỗi và nó tạo ra một lưu tự động. Tự động lưu bị tắt cho đến khi bạn đi tới cài đặt và nhấp vào ok'), mtWarning, mbOk)
        return false
      end
    end
  end
  ...
  if path==nil then
    messageDialog(translate('Không tìm được vị trí để lưu dữ liệu tự động lưu'), mtError, mbOk)  
    return false
  end
    
  path=path..'Cheat engine lưu tự động';
  lfs.mkdir(path)
  ...
  if (r==nil) or (r==false) then
    if err==nil then
      err=translate('Lý do không xác định')    
    end      
    messageDialog(translate('Không thể tự động lưu bảng : ')..err, mtError, mbOk)
    return false
  end
  ...
  if cestate==nil then
    messageDialog(translate('Không thể tự động lưu trạng thái : ')..err, mtError, mbOk)
    return false
  end
...
function autosave.loadState()
 -- print("loadState()")
  ...
  AutoSaveSettings['ProcessID']=getCheatEngineProcessID()
  local path=autosave.getPath()..'Cheat engine lưu tự động\\';  
  ...
  if (r==nil) or (r==false) then
    ...
    messageDialog(string.format(translate('Không thể tải lưu tự động tại %s  . Lỗi: %s'), path, err), mtError, mbOk)
    return false
  end
  ...
  if cestate then  
    ...
    if version~=AutoSaveVersion then
      ...
      messageDialog(translate('Trạng thái đã lưu thuộc về một phiên bản tự động lưu khác hiện không được triển khai'), mtError, mbOk)  
      return false
    end
...
autosave.settingsLblInterval.Caption=translate('Khoảng thời gian lưu tự động (tính bằng giây. 0 là tắt)')
autosave.settingsLblInterval.Align='alTop'

local insertNode=sf.SettingsTreeView.Items[3]
local node=sf.SettingsTreeView.Items.insert(insertNode, translate("Lưu tự động"))
...
if (pid) and (pid ~= '') and (getProcessList()[tonumber(pid)]==nil) then
  ...
  if messageDialog(translate('Lần trước cheat engine đã không đóng đúng cách. Bạn có muốn khôi phục lại công việc của mình?'), mtInformation, mbYes, mbNo)==mrYes then
    autosave.loadState()    
  end
end
