...
pmAddToNewGroup.Caption=translate('Thêm vào nhóm mới')
...
pmAddToNewGroup.OnClick=function(s)
  ...
  if AddressList.SelCount==0 then
    messageDialog('Vui lòng chọn ít nhất một mục trước', mtError, mbOK)
    return
  end
  ...
  local groupname=translate(string.format('Nhóm %d',count+1))
  if (isKeyPressed(VK_CONTROL)==false) then
    groupname=InputQuery(translate('Nhóm'), translate('Bạn muốn tên nhóm là gì?'), groupname)
    if groupname then
      if hasAddressSupport then
        withAddress=messageDialog(translate('Bạn có muốn phiên bản "địa chỉ"?'), mtConfirmation, mbYes, mbNo)==mrYes
      end
    else
      return
    end
